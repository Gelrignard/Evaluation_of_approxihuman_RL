...
class SplineTrack():
    ...
    def __init__(
        ...
        if path_to_file is not None:
            ...
            raise NotImplementedError("neither coordinates or the path to the coordinates was given! I cannot just come up with a track!")
        ...
        if path_to_traj is not None:
            ...
            self.delta_traj = self.trajectory.find_theta_slow(
                self.midline.get_coordinate(0)
                )
        else:
            self.trajectory = self.midline
    ...
    def get_angle(self, theta, line = 'mid') -> float:	
        ...
        if line == 'mid':
            return self.midline.get_angle(theta)  
        elif line == 'int':
    ...
    def get_coordinate(self, theta, line = 'mid') -> np.array:
        ...
        if line == 'mid':
            coord = self.midline.get_coordinate(theta)
        elif line == 'int':
    ...
    def get_derivative(self, theta, line = 'mid'):
        ...
        if line == 'mid':
            der = self.midline.get_derivative(theta)
        elif line == 'int':
    ...
    def get_curvature(self, theta, line='mid'):
        ...
        if line == 'mid':
            curvature = self.midline.get_curvature(theta)
        elif line == 'int':
    ...
    def get_dphi_dtheta(self, theta, line='mid'):
        ...
        if line == 'mid':
            derivative = self.midline.get_dphi_dtheta(theta)
        elif line == 'int':
    ...
    def find_theta(self, coord: np.ndarray, theta_est, eps: float = 0.01):
        ...
        min_theta = self.midline.find_theta(coord, theta_est, eps)
    ...
    def find_theta_slow(self, coord: np.ndarray, eps: float = 0.1):
        ...
        min_theta = self.midline.find_theta_slow(coord, eps)

        return min_theta    

    def is_coord_behind(self, coord: np.ndarray, theta: float):
        return self.midline.is_coord_behind(coord, theta)
...
class SplineTrackNew():
    ...
    def __init__(
        ...
        self.midline = EnhancedSpline(coords, params, self.track_length, lame=False)
        self.left_widths = Spline(params,left_widths)
        self.right_widths = Spline(params,right_widths)
